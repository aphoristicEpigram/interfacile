#!/usr/bin/env python3
"""Precondition helper for the fix-ticket workflow.

Usage:
    python3 fix_ticket.py EM-XXXX

Checks:
- ticket exists and is OPEN
- a Pre-Work Staff Review block exists with verdict VALID — proceed
- all depends_on tickets are CLOSED
- files in Files Touched exist (or flags missing ones)

Outputs an implementation-plan scaffold if preconditions pass.
"""
...
def has_valid_staff_review(path: Path) -> tuple[bool, str]:
    """Return (ok, reason)."""
    text = path.read_text(encoding="utf-8")
    block_match = re.search(
        r"##\s*Pre-Work\s*Staff\s*Review(.*?)\n## ",
        text + "\n## ",
        re.DOTALL | re.IGNORECASE,
    )
    if not block_match:
        return False, "No `## Pre-Work Staff Review` block found."
    block = block_match.group(1)
    # Match "**Verdict:** VALID — proceed" (bold is around the label, not the value).
    if re.search(
        r"\*\*Verdict:\*\*\s*VALID\s*[-–—]\s*proceed",
        block,
        re.IGNORECASE,
    ):
        return True, "Verdict: VALID — proceed"
    if re.search(r"Verdict", block, re.IGNORECASE):
        return False, f"Staff review block found but verdict is not VALID — proceed.\n\n{block.strip()[:500]}"
    return False, "Staff review block found but missing a Verdict line."
...
def main() -> int:
    if len(sys.argv) != 2:
        print("Usage: fix_ticket.py EM-XXXX", file=sys.stderr)
        return 1
    ...
    # Check 2: no valid staff review. The fix skill will auto-run /scope if missing.
    review_ok, review_reason = has_valid_staff_review(ticket_path)
    if not review_ok:
        print(f"⚠️  No valid staff review found for {ticket_id}.\n")
        print(f"The /fix skill will run /scope {ticket_id} first, then continue with implementation.\n")
        print(f"Reason: {review_reason}")
        return 1

    # Halt 3: open dependencies.
