...
def load_config(repo_root):
    """Read <repo_root>/interfacile.json. {} when absent; a malformed file is
    warned about and ignored so a bad config never takes the dashboard down."""
    path = os.path.join(repo_root, "interfacile.json")
    try:
        ...
        sys.stderr.write("interfacile.json ignored (%s)\n" % exc)
        return {}
...
def apply_config(conf):
    ...
    FAVICON = brand.get("favicon", FAVICON)
    HEADER_ICON = brand.get("icon", brand.get("favicon", HEADER_ICON))
    EYEBROW = brand.get("eyebrow", EYEBROW)
    TAGLINE = brand.get("tagline", TAGLINE)

    ids = conf.get("ids", {})
    PFX = ids.get("prefix", PFX)
    ID_DIGITS = int(ids.get("digits", ID_DIGITS))
    _IDRE = make_id_res(PFX, ID_DIGITS)
    TICKET_ID_RE = _IDRE["ticket_id"]
    EPIC_CODE_RE = _IDRE["epic_code"]
    TICKET_PARTS_RE = _IDRE["ticket_parts"]
    ...
    SERVER_PORT = int(conf.get("server", {}).get("port", SERVER_PORT))
...
def main():
    ...
    ap.add_argument("--repo", default=None,
                    help="target repo root to scan (overrides $TICKET_DASHBOARD_REPO; "
                         "default: the repo this script lives in)")
    args = ap.parse_args()

    global REPO_ROOT, TICKETS_DIR, ADR_DIR, PINS_FILE, NOTE_FILES
    if args.repo:
    ...
    apply_config(load_config(REPO_ROOT))

    if not os.path.isdir(TICKETS_DIR):
        sys.exit("tickets/ not found at %s" % TICKETS_DIR)

    port = args.port if args.port is not None else int(os.environ.get("PORT", SERVER_PORT))
    url = "http://%s:%d/" % (args.host, port)
    srv = ThreadingHTTPServer((args.host, port), Handler)
    print("%s · interfacile  ->  %s" % (BRAND, url))
    print("scanning: " + TICKETS_DIR)
